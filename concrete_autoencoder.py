...
class ConcreteSelect(Layer):
    ...
    def build(self, input_shape):
        ...
        self.thresh = self.add_weight(
            name="thresh", shape=[], initializer=Constant(3.0), trainable=False
        )
...
class StopperCallback(EarlyStopping):
    ...
    def __init__(self, mean_max_target=0.998):
        ...
        super(StopperCallback, self).__init__(
            monitor="",
            patience=float("inf"),
            verbose=1,
            mode="max",
    ...
    def on_epoch_begin(self, epoch, logs=None):
        ...
        print(
            "regularization loss:",
            K.get_value(self.model.get_layer("concrete_select").regularization_loss()),
        )
    ...
    def get_monitor_value(self, logs):
        monitor_value = K.get_value(
            K.mean(
                K.max(
                    K.softmax(self.model.get_layer("concrete_select").logits), axis=-1
                )
            )
        )
...
class ConcreteAutoencoderFeatureSelector:
    ...
    def fit(self, X, Y=None, val_X=None, val_Y=None):
        ...
        for i in range(self.tryout_limit):
            ...
            thresh_alpha = math.exp(
                math.log(1.0 / 3.0) / (num_epochs * steps_per_epoch)
            )
            ...
            if self.class_weights is not None:
                self.class_weights = dict(
                    zip(np.arange(0, X.shape[1]), self.class_weights)
                )
            else:
                self.class_weights = dict(
                    zip(np.arange(0, X.shape[1]), np.ones(1, X.shape[1]))
                )
            ...
            self.model.compile(
                ...
                loss=lambda y_true, y_pred: BinaryCrossentropy(from_logits=False)(
                    y_true, y_pred
                )
                + self.concrete_select.regularization_loss(),
            ...
            if (
                K.get_value(
                    K.mean(K.max(K.softmax(self.concrete_select.logits, axis=-1)))
                )
                >= stopper_callback.mean_max_target
            ):
                break
    ...
    def get_mask(self):
        return K.get_value(
            K.sum(
                K.one_hot(
                    K.argmax(self.model.get_layer("concrete_select").logits),
                    self.model.get_layer("concrete_select").logits.shape[1],
                ),
                axis=0,
            )
        )
